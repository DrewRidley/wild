...
//! with debug assertions. The integration test harness drives them via the `//#Malfunction:<name>`
//! directive; a config using that directive fails if linker-diff reports *no* difference, which is
//! what makes each malfunction a proof that some check really does detect something.
//!
//! ## Inventory
//!
//! ELF:
